...
plot f every 1::16 using (($0)*Ts*1000):2 lt -1 with line
...
plot f every 1::16 using (($0)*Ts*1000):3 lt -1 with line,\
     f every 1::16 using (($0)*Ts*1000):4 lt -1 lc rgb "orange" with line,\
     f every 1::16 using (($0)*Ts*1000):5 lt -1 lc rgb "gray" with line,\
     f every 1::16 using (($0)*Ts*1000):6 lt -1 lc rgb "blue" with line
...
stats f using 7 every ::16 #nooutput

set label 1 "{/Symbol:Italic t} [Nm]" at graph 0.02,0.85 left
set label 2 sprintf("{/Symbol:Italic t}_{p-p} = %1.2f[Nm]", (STATS_max-STATS_min)/10.0*5.0) at graph 0.22,0.85 left
...
set ytics 2
set mytics 2
set xlabel "Time {/Italict t} [ms]"offset 0,0.5
set ylabel "{/Symbol:Italic t} [Nm]" offset 1.4,0
#set ylabel offset 0,-1
plot f every 1::16 using (($0)*Ts*1000):($7/10.0*5.0) lt -1 with line,\
